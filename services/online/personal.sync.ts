...
export const personalSync = {
  ...
  async push() {
    ...
    try {
      ...
      console.warn('[personalSync] No se pudieron cargar los roles para el mapeo:', e);
    }
    ...
    for (const record of pending) {
      try {
        ...
        console.log(`Usuario ${record.email} sincronizado exitosamente vía API.`);
      } catch (err: any) {
        if (err.response?.status === 400 || err.response?.status === 409) {
          await db.update(users).set({ is_synced: true }).where(eq(users.id, record.id));
          console.log(`Usuario ${record.email} ya existía en el remoto. Marcado como sincronizado.`);
        } else if (err.response?.status === 422) {
          console.error(`Error de validación (422) para ${record.email}:`, JSON.stringify(err.response.data.detail, null, 2));
        } else {
          console.error(`Error de sincronización para usuario ${record.email}:`, err.message || err);
        }
      }
    }
  },
  ...
  async pull() {
    try {
      ...
      for (const record of usersList) {
        ...
        const currentRoleId = record.role_id || record.roleId || (record.role && record.role.id) || 'TRABAJADOR';
        ...
        const userData = {
          ...
          password_hash: record.password_hash || record.passwordHash || 'API_NO_PASSWORD',
          role_id: currentRoleId,
          status: record.status || 'ACTIVO',
          suspended_from: record.suspended_from || record.suspendedFrom ? new Date(record.suspended_from || record.suspendedFrom).toISOString() : null,
          suspended_until: record.suspended_until || record.suspendedUntil ? new Date(record.suspended_until || record.suspendedUntil).toISOString() : null,
        ...
        try {
          ...
          if (insertErr.message?.includes('UNIQUE constraint failed')) {
            console.log(`[personalSync] Conflicto de unicidad detectado para ${record.email}. Reintentando actualización manual.`);
